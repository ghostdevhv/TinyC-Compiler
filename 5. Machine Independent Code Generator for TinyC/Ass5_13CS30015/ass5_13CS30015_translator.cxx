...
// symbol_table *curr_sym_table = glob_sym_table;
// symbol_table * curr_sym_table = glob_sym_table;
...
void quad::print()
{
	switch(op) {
        ...
        case(opcode_dec_ASSIGN):
            printf("%s = %s\n", result.c_str(), arg1.c_str());
            break;
        case(opcode_dec_IF_EXPRESSION):
            printf("if (%s) goto %s\n", arg1.c_str(), result.c_str());
            break;
        case(opcode_dec_IF_NOT_EXPRESSION):
            printf("ifFalse (%s) goto %s\n", arg1.c_str(), result.c_str());
            break;
        case(opcode_dec_LESS_THAN):
            printf("if (%s < %s) goto %s\n", arg1.c_str(), arg2.c_str(), result.c_str());
            break;
        case(opcode_dec_LESS_THAN_EQUAL):
            printf("if (%s <= %s) goto %s\n", arg1.c_str(), arg2.c_str(), result.c_str());
            break;
        case(opcode_dec_GREATER_THAN):
            printf("if (%s > %s) goto %s\n", arg1.c_str(), arg2.c_str(), result.c_str());
            break;
        case(opcode_dec_GREATER_THAN_EQUAL):
            printf("if (%s >= %s) goto %s\n", arg1.c_str(), arg2.c_str(), result.c_str());
            break;
        case(opcode_dec_EQUAL_EQUAL_TO):
            printf("if (%s == %s) goto %s\n", arg1.c_str(), arg2.c_str(), result.c_str());
            break;
        case(opcode_dec_NOT_EQUAL_TO):
            printf("if (%s != %s) goto %s\n", arg1.c_str(), arg2.c_str(), result.c_str());
            break;
...
void conv2Bool(expression_type * e) 
{
    if(e->type->type_d != type_dec_BOOL) 
    {
        ...
        quad_arr.emit(quad("", e->loc->name, opcode_dec_EQUAL_EQUAL_TO,"0"));
...
bool checkParams(expression_type e, vector <expression_type *> * args) 
{
    ...
    for(int i = 1; i < (int)v.size(); ++i) 
    {
        ...
        expression_type e1;
        e1.type = v[i]->type;
        expression_type * e2 = (*args)[paramN];
        ++paramN;
